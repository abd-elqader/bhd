                ...
                <div class="col-12 col-md-6">
                    <div class="m-2">
                        <form method="POST" action="{{ route('client.address.update', $address['id']) }}">
                            ...
                            <div class="my-2">
                                <span class="main_bold p-2 {{ $country_id != 2 ? 'd-none' : '' }} BahrainCountry">@lang('website.region')</span>
                                <span class="main_bold p-2 {{ $country_id == 2 ? 'd-none' : '' }} OtherCountries">@lang('website.city')</span>
                                <select name="region_id" id="region_id" class="border rounded p-2 w-100">
                                    <option value="" disabled hidden selected>@lang('messages.Select')</option>
                                    @foreach($regions as $region)
                                        <option @selected( $address['region_id'] == $region['id']) value="{{ $region['id'] }}">{{ $region['title_' . lang()] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="my-2">
                                <span class="main_bold p-2 {{ $country_id != 2 ? 'd-none' : '' }} BahrainCountry">@lang('website.block')</span>
                                <span class="main_bold p-2 {{ $country_id == 2 ? 'd-none' : '' }} OtherCountries">@lang('website.district')</span>
                                <input type="text" name="district" class="border rounded p-2 w-100 OtherCountries" value="{{ $address['block'] }}">
                                <input type="hidden" name="block_id" id="block_id" class="border rounded p-2 w-100 BahrainCountry" value="{{ $address['block_id'] }}">
                                <select name="block" id="block" class="border rounded p-2 w-100 BahrainCountry">
                                    <option value="" disabled hidden selected>@lang('messages.Select')</option>
                                    @foreach($blocks as $block)
                                        <option data-uuid="{{ $block['uuid'] }}" {{ $address['block'] == $block['id'] ? 'selected' : '' }} value="{{ $block['id'] }}">{{ $block['number'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="my-2">
                                <span class="main_bold p-2 {{ $country_id != 2 ? 'd-none' : '' }} BahrainCountry">@lang('website.road')</span>
                                <span class="main_bold p-2 {{ $country_id == 2 ? 'd-none' : '' }} OtherCountries">@lang('website.street')</span>
                                <input type="number" min="1" max="99999" name="road" class="border rounded p-2 w-100" value="{{ $address['road'] }}">
                            </div>
                            <div class="my-2">
