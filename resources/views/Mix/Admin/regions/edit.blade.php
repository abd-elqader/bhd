...
@section('pagetitle',__('messages.regions'))
@section('content')
<form method="POST" action="{{ route('admin.country.regions.update',['country' => request('country'),'region'=>$Region]) }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <input readonly type="hidden"  value="{{ $Region['lat'] ?? 26.227934462972144 }}" name="lat" id="lat" class="form-control" required>
        <input readonly type="hidden"  value="{{ $Region['long'] ?? 50.58910840410498 }}" name="long" id="long" class="form-control" required>
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control  select2me" required id="visibility" name="status">
                <option {{ $Region['status'] == 1 ? 'selected' : '' }} value="1">@lang('dashboard.visible')</option>
                <option {{ $Region['status'] == 0 ? 'selected' : '' }} value="0">@lang('dashboard.hidden')</option>
            </select>
        </div>
