...
@section('content')
<form method="POST" action="{{ route('admin.themes.update', $Theme->id)}}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="col-sm-12 col-md-6">
            ...
            <ul class="selected_pages  py-2 my-2">
                @foreach ($Theme->themePages as $page)
                    <li class="li-comp li-{{$page->id}} {{$page->type}}comp"><h3 class="pb-2" style="opacity:70%">{{$page->type}}</h3><input name="themePages[]" type="text" hidden value="{{$page->id}}"><a target="_blanck" href="/previewFullThemePage/{{$page->id}}">{{$page->title()}}</a></li>
                @endforeach
            </ul>
        </div>
            ...
            <ul class="pages py-4 my-4">
                @foreach($ThemePages->unique('type') as $item)
                    ...
                    @foreach ($ThemePages->where('type',$item->type) as $page)
                        <li>
                            <input class="{{$item->type}}box" type="checkbox" value="{{ $page->id }}" data-order="{{$page->order}}" data-type="{{$item->type}}" @if(in_array($page->id, $Theme->themePages->pluck('id')->toArray())) checked @endif>
