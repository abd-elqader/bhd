...
<form action="{{ route('admin.agents.update',$Agent) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-6 ">
            ...
            <input value="{{ $Agent->name }}" type="text" name="name" id="name" parsley-trigger="change" required value="" class="form-control">
        </div>
        <div class="col-md-6 col-sm-12">
            <label for="email">{{ __('website.email') }}</label>
            <input value="{{ $Agent->email }}" type="email" name="email" parsley-trigger="change" value="" required class="form-control">
        </div>
        <div class="col-md-6 col-sm-12">
            <label for="phone">{{ __('website.phone') }}</label>
            <input  class="form-control @error('phone_code') is-invalid @enderror @error('country_code')is-invalid @enderror @error('phone')is-invalid @enderror" type="phone" name="phone" id="phone_number" placeholder="{{ __('messages.Phone') }}" value="{{ old('phone', $Agent->phone) }}" required>
            <input type="hidden" name="country_code" id="country_code" value="{{ $Agent->country_code  }}">
            <input type="hidden" name="phone_code" id="phone_code" value="{{ $Agent->phone_code }}">
        </div>
        <div class="col-md-6 col-sm-12">
            <label for="role_id">{{ __('messages.role') }}</label>
            <select class="form-control" id="role_id" name="role_id">
                @foreach ($roles as $item)
                <option {{ $Agent->hasRole($item->name) ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                @endforeach
            </select>
        </div>
