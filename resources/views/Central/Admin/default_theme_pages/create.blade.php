...
@section('content')
<form id="mainForm" method="POST" action="{{ route('admin.default_theme_pages.store') }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    <div class="row">

        <div class="col-sm-12 col-md-6 pb-4 mb-4">
            <h2 id="title_ar" style="cursor: pointer">@lang('dashboard.title_ar')</h2>
            <input required type="text" name="title_ar" placeholder="@lang('dashboard.title_ar')" required class="form-control mt-2">
        </div>

        <div class="col-sm-12 col-md-6 pb-4 mb-4">
            <h2 id="title_en" style="cursor: pointer">@lang('dashboard.title_en')</h2>
            <input required type="text" name="title_en" placeholder="@lang('dashboard.title_en')" required class="form-control mt-2">
        </div>
        ...
        <div class="col-sm-12 col-md-7 pb-4 mb-4 mx-auto">
            ...
            <select required id="typePicker" name="type" class="form-control mt-2" required>
                <option selected hidden disabled value="">@lang('messages.Select')</option>
                <option value="Home">@lang('website.home')</option>
                <option value="Product">@lang('dashboard.product')</option>
                <option value="Category">@lang('messages.categories')</option>
...
@section('js')
    <script>
        ...
        $(document).on('click', '.submitbtn', function(){
            console.log();
            if($('.CR:checked').length > 0 && $('#typePicker  option:selected').val()){
                $('#mainForm').submit();
            }else{
                alert('All Fildes And CopyRights are required');
            }
        });
