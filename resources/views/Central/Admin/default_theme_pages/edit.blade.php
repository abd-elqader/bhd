...
@section('content')
<form id="mainForm" method="POST" action="{{ route('admin.default_theme_pages.update', $Theme->id) }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="col-sm-12 col-md-7 pb-4 mb-4 mx-auto">
            ...
            <select required name="type" class="form-control mt-2" >
                ...
                <option {{$Theme->type == 'Category' ? 'selected' : ''}} value="Category">@lang('messages.categories')</option>
            ...
            <ul class="components py-4 my-4">
                @foreach($Components->unique('type') as $item)
                    ...
                    @foreach ($Components->where('type',$item->type) as $component)
                        <li>
                            <input class="componentInput {{$item->type}}box @if($item->type == 'Copyright') CR @endif" type="checkbox" value="{{ $component->id }}" data-order="{{$component->row_id}}" data-type="{{$item->type}}" @if(in_array($component->id, $Theme->components->pluck('id')->toArray())) checked @endif data-pages="{{$item->pages}}">
...
@section('js')
    <script>
        ...
        $(document).on('click', '.components input[type="checkbox"]', function() {
            ...
            if($(this).is(':checked')){
                $("." + type + "comp").remove();
                $(".selected_components").append('<li data-order="'+ order +'" class="li-comp li-'+id+ ' ' + type + 'comp"><h3 class="pb-2" style="opacity:70%">' + type + '</h3><input name="components[]" type="text" hidden value="'+id+'"><a target="_blanck" href="/previewComponent/'+id+'">'+text+'</a></li>');
        ...
        $(document).on('click', '#preview', function() {
            var route = "{{ route('previewDefaultThemePage') }}";
            var ids = [];
