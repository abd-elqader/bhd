...
@section('content')
<form method="POST" action="{{ route('admin.default_themes.store') }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    <div class="row">
...
@section('js')
    <script>
        $(document).on('click', '.pages input[type="checkbox"]', function() {
            ...
            if($(this).is(':checked')){
                $("." + type + "comp").remove();
                $(".selected_pages").append('<li data-order="'+ order +'" class="li-comp li-'+id+ ' ' + type + 'comp"><h3 class="pb-2" style="opacity:70%">' + type + '</h3><input name="themePages[]" type="text" hidden value="'+id+'"><a target="_blanck" href="/previewDefaultFullThemePage/'+id+'">'+text+'</a></li>');


                $('.li-comp').sort(function(a, b){
                    return ($(b).data('order')) < ($(a).data('order')) ? 1 : -1;
                })
                .appendTo('.selected_pages');
